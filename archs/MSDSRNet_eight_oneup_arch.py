...
class MSDSRNet_eight_oneup(nn.Module):
    def __init__(self,
                 in_nc=3,
                 nf=64,
                 num_modules=4,
                 ...
                 deploy=False):
        super(MSDSRNet_eight_oneup, self).__init__()
        ...
        for i in range(self.nBlocks):
            print(' ********************** Block {} '
                  ' **********************'.format(i + 1))
            m = _build_block(nIn, nf, self.steps, n_layer_curr, deploy=deploy)
            self.blocks.append(m)
            n_layer_curr += self.steps


        self.upsample.append(self._build_upsample(nf, upscale))
        # self.upsample = self._build_upsample(nf, upscale)
 ...
 # ------------------------------------------------#
    ...
    # def _build_block(self, in_nc, nf, step, n_layer_curr):

    #         layers =  []
    #         for i in range(step):
    #             n_layer_curr += 1

    #             layers.append(B.RFDB(in_channels=nf))

    #         layers.append(B.conv_block(nf * step, nf, kernel_size=1, act_type='lrelu'))
    #         layers.append(B.conv_layer(nf, nf, kernel_size=3))
